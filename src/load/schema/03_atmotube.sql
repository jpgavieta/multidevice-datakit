...
CREATE TABLE IF NOT EXISTS fitbit.readings (
    ...
    end_at         TIMESTAMPTZ,         -- NULL for "sample" grain (single instant, no interval)
...
-- fitbit_parser._parse_sleep() emits "started_at" per stage row, plus "device_id"
-- and "session_started_at" as join keys only — load.py resolves session_started_at
-- -> the parent fitbit.sleep_sessions.id (via its own started_at) and drops both
-- join-key fields before inserting; neither is a column here.
